...
def detect_user_file_type(filepath: Path) -> str:
    ...
    try:
        ...
        raise DataLoadError("Failed to detect format", context={'file': str(filepath), 'error': str(e)})
...
def load_23andme_file(filepath: Path) -> pd.DataFrame:
    ...
    try:
        ...
        if len(df) == 0:
            raise ValidationError("No valid variants after quality check", context={'file': str(filepath)})
        ...
        raise DataLoadError("Failed to load 23andMe", context={'file': str(filepath), 'error': str(e)})
...
def load_user_tsv(filepath: Path) -> pd.DataFrame:
    ...
    try:
        ...
        if miss:
            raise ValidationError(f"Missing required columns: {miss}", 
