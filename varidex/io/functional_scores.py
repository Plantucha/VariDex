...
class FunctionalScoreLoader:
            ...
            with open(eve_path, 'r') as f:
                for line in f:
                    if gene in line and variant in line:
                        ...
                        if eve_score < PS3_THRESHOLDS['eve']:  # Lower is more deleterious
                            return FunctionalScore(
                                source='EVE',
                                score=eve_score,
                                score_type='conservation'
                            )
            return None
        except Exception as e:
    ...
    def aggregate_scores(
        ...
        if am_score: all_scores.append(am_score)
        
        esm_score = self.load_esm1v(gene, variant)
        if esm_score: all_scores.append(esm_score)
        
        eve_score = self.load_eve(gene, variant)
        if eve_score: all_scores.append(eve_score)
        ...
        # Decision logic
        supporting = [s for s in all_scores if s.evidence_level in ['strong', 'moderate']]
        total_sources = len(all_scores)
        supporting_sources = len(supporting)
        
        if supporting_sources >= 2 or any(s.evidence_level == 'strong' for s in all_scores):
            strength = 'Strong'
