...
def annotate_with_gnomad(
    ...
    try:
        from varidex.integrations.gnomad.query_parallel import annotate_with_gnomad_parallel
        
        # Convert DataFrame to list of dicts
        variants = df[['chromosome', 'position', 'ref_allele', 'alt_allele']].to_dict('records')
        ...
        with GnomADQuerier(gnomad_dir) as querier:
            for idx, row in tqdm(df.iterrows(), total=len(df), desc="Querying gnomAD", unit="var"):
                try:
...
def apply_frequency_acmg_criteria(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["BS1"] = (df["gnomad_af"] > 0.01) & (df["gnomad_af"] <= 0.05)  # 1-5% = Benign strong
