...
class StageProfiler:
    ...
    def end_stage(self, context: Dict, output_rows: int = 0, status: str = "success", error: str = None):
        ...
        metric = StageMetrics(
            stage_id=context['stage_id'], stage_name=context['stage_name'],
            status=status, duration_sec=round(duration, 3),
            memory_mb=round(memory, 2), cpu_percent=round(cpu, 1),
            input_rows=context['input_rows'], output_rows=output_rows, error=error
        )

        with self._lock:  # OPTIMIZED: thread-safe
            self.metrics.append(metric)
        logger.info(f"⏱ {metric.stage_name}: {metric.duration_sec}s, {metric.memory_mb}MB, {metric.output_rows} rows")
...
class CheckpointManager:
    ...
    def save_checkpoint(self, stage_id: int, df: pd.DataFrame, stage_name: str) -> Optional[pd.DataFrame]:
        if not self.enabled:
            return df
        ...
        checkpoint = StageCheckpoint(
            stage_id=stage_id, timestamp=time.time(),
            row_count=len(df), file_path=file_path  # OPTIMIZED: no hash
        )
...
class StageExecutor:
    ...
    def __init__(self, profiler: StageProfiler, checkpoint_manager: CheckpointManager,
                 dry_run: bool = False, skip_stages: Set[int] = None):
    ...
    def execute(self, stage_id: int, stage_name: str, stage_func, *args, **kwargs):
        ...
        ctx = self.profiler.start_stage(stage_id, stage_name, input_rows=kwargs.get('input_rows', 0))
...
def execute_stage2_load_clinvar(clinvar_file: Path, checkpoint_dir: Path, loader: Any, safeguard_config: Dict) -> pd.DataFrame:
    ...
    if user_type == '23andme':
        ...
        if hasattr(loader, 'load_vcf_file') and loader.load_vcf_file:
            ...
            if '#CHROM' in user_df.columns:
                user_df.rename(columns={'#CHROM': 'chromosome', 'POS': 'position',
                                       'ID': 'rsid', 'REF': 'ref', 'ALT': 'alt'}, inplace=True)
    else:
        user_df = pd.read_csv(user_file, sep='\t', low_memory=False)
    logger.info(f"✓ Loaded {len(user_df):,} user variants")
    return user_df

def execute_stage4_hybrid_matching(clinvar_df: pd.DataFrame, user_df: pd.DataFrame,
                                   clinvar_type: str, user_type: str, loader: Any, safeguard_config: Dict) -> pd.DataFrame:
...
def execute_stage5_acmg_classification(matched_df: pd.DataFrame, safeguard_config: Dict,
                                       clinvar_type: str, user_type: str,
                                       parallel: bool = True, batch_size: int = 1000) -> Tuple[List, Dict]:
    ...
    if parallel and len(matched_df) > batch_size:
        ...
        batches = [matched_df.iloc[i:i+batch_size] for i in range(0, len(matched_df), batch_size)]
        ...
        with ProcessPoolExecutor(max_workers=4) as executor:
            with tqdm(total=len(batches), desc="Classifying batches", unit="batch") as pbar:
                futures = {executor.submit(_classify_batch, data): i for i, data in enumerate(batch_data)}
        ...
        with tqdm(total=len(matched_df), desc="Classifying variants", unit="var") as pbar:
            classified_variants, stats = classify_variants_production(matched_df, safeguard_config, clinvar_type, user_type)
...
def execute_stage7_generate_reports(results_df: pd.DataFrame, stats: Dict, output_dir: Path, reports: Any) -> Dict[str, Path]:
...
def execute_stages_2_3_parallel(clinvar_file: Path, user_file: Path, checkpoint_dir: Path,
                                loader: Any, user_type: str, safeguard_config: Dict, max_workers: int = 2) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Execute Stages 2 and 3 in parallel."""
    logger.info("🔀 Starting parallel execution: Stages 2 & 3")
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_clinvar = executor.submit(execute_stage2_load_clinvar, clinvar_file, checkpoint_dir, loader, safeguard_config)
...
if __name__ == '__main__':
    ...
    stages = [(2, execute_stage2_load_clinvar), (3, execute_stage3_load_user_data),
              (4, execute_stage4_hybrid_matching), (5, execute_stage5_acmg_classification),
              (6, execute_stage6_create_results), (7, execute_stage7_generate_reports)]
