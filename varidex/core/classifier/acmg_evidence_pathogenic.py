...
LOF_CONSEQUENCES = frozenset([
    'frameshift_variant', 'frameshift', 'stop_gained', 'stop_gain',
    'splice_acceptor_variant', 'splice_donor_variant',
    'transcript_ablation', 'start_lost', 'initiator_codon_variant',
    'nonsense', 'canonical_splice'
])

# Missense consequence terms
MISSENSE_CONSEQUENCES = frozenset([
    'missense_variant', 'missense', 'nonsynonymous_snv'
])

# Inframe indel terms
INFRAME_CONSEQUENCES = frozenset([
    'inframe_deletion', 'inframe_insertion', 'in_frame_deletion',
    'in_frame_insertion', 'stop_lost'
])
...
class PathogenicEvidenceAssigner:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(f"PathogenicEvidenceAssigner initialized with {sum([
    ...
    def check_pp3(self, variant: Dict[str, Any], prediction_scores: Optional[Dict] = None) -> bool:
        ...
        try:
            ...
            if total_predictors >= 3 and deleterious_count >= (total_predictors * 0.67):
                logger.info(f"PP3: {deleterious_count}/{total_predictors} predictors agree on deleterious")
                return True
