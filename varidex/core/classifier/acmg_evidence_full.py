...
class DataRequirements:
    ...
    functional_study_result: Union[str, FunctionalStudyResult, None] = (
        None  # Accepts enum or string
    )
...
class ACMGEvidenceEngine:
    ...
    def pvs1(
        self, variant_type: str, consequence: str, gene: str, data: DataRequirements
    ) -> EvidenceResult:
        """PVS1: Null variant in LOF-intolerant gene."""
        lof_types = {
            "frameshift",
        ...
        return EvidenceResult(
            "PVS1",
            False,
            "Not LOF in LOF gene (gene={gene}, type={variant_type})",
            0.0,
            True,
        )

    def ps1(
        self, gene: str, aa_change: Optional[str], data: DataRequirements
    ) -> EvidenceResult:
        ...
        return EvidenceResult(
            "PS1", False, "Different AA change (checked, no match)", 0.0, True
        )
    ...
    def ps2(self, data: DataRequirements) -> EvidenceResult:
        ...
        if data.de_novo_confirmed is None:
            return EvidenceResult(
                "PS2", False, "No parental data available", 0.0, False
            )

        if data.de_novo_confirmed:
            return EvidenceResult(
                "PS2", True, "Confirmed de novo with parental testing", 1.0, True
            )
    ...
    def ps3(self, data: DataRequirements) -> EvidenceResult:
        ...
        if not data.functional_study_result:
            return EvidenceResult("PS3", False, "No functional study data", 0.0, False)
    ...
    def ps4(self, data: DataRequirements) -> EvidenceResult:
        ...
        return EvidenceResult(
            "PS4", False, "Case-control data not available", 0.0, False
        )
    ...
    def pm1(self, data: DataRequirements) -> EvidenceResult:
        ...
        if data.in_functional_domain is None:
            return EvidenceResult(
                "PM1", False, "Functional domain data not available", 0.0, False
            )
    ...
    def pm2(self, data: DataRequirements) -> EvidenceResult:
        ...
        if data.gnomad_af is None and data.exac_af is None:
            return EvidenceResult(
                "PM2", False, "Population frequency data not available", 0.0, False
            )
        ...
        return EvidenceResult(
            "PM2", False, "Present in population (AF={af})", 0.0, True
        )
    ...
    def pm3(self, data: DataRequirements) -> EvidenceResult:
        ...
        if data.is_in_trans_pathogenic is None:
            return EvidenceResult("PM3", False, "Phase data not available", 0.0, False)

        if data.is_in_trans_pathogenic:
            return EvidenceResult(
                "PM3", True, "In trans with pathogenic variant", 0.9, True
            )
    ...
    def pm4(self, consequence: str) -> EvidenceResult:
        ...
        if has_change:
            return EvidenceResult(
                "PM4", True, "Protein length changing variant", 0.75, True
            )
    ...
    def pm5(self, aa_change: Optional[str], data: DataRequirements) -> EvidenceResult:
        ...
        return EvidenceResult(
            "PM5", False, "Pathogenic variant database not available", 0.0, False
        )
    ...
    def pm6(self, data: DataRequirements) -> EvidenceResult:
        ...
        if data.de_novo_assumed:
            return EvidenceResult(
                "PM6", True, "Assumed de novo (no parental confirmation)", 0.6, True
            )
    ...
    def pp1(self, data: DataRequirements) -> EvidenceResult:
        ...
        return EvidenceResult(
            "PP1", False, "Segregation analysis not implemented", 0.0, False
        )
    ...
    def pp2(self, consequence: str, gene: str) -> EvidenceResult:
        ...
        if is_missense and is_constrained:
            return EvidenceResult(
                "PP2", True, "Missense in constrained gene {gene}", 0.7, True
            )
    ...
    def pp3(self, data: DataRequirements) -> EvidenceResult:
        ...
        if not scores:
            return EvidenceResult(
                "PP3", False, "No computational predictions", 0.0, False
            )
        ...
        if deleterious_count >= 2:
            ...
            return EvidenceResult(
                "PP3",
                True,
                "Multiple tools predict deleterious ({details})",
                0.65,
                True,
            )
    ...
    def pp4(self, data: DataRequirements) -> EvidenceResult:
        ...
        if data.patient_phenotype_specific:
            return EvidenceResult(
                "PP4", True, "Patient phenotype specific for this gene", 0.7, True
            )
    ...
    def pp5(self, clinvar_sig: str) -> EvidenceResult:
        ...
        if not clinvar_sig or not clinvar_sig.strip():
            return EvidenceResult(
                "PP5", False, "No ClinVar significance data", 0.0, False
            )
        ...
        if any(term in clinvar_sig.lower() for term in path_terms):
            return EvidenceResult(
                "PP5", True, "ClinVar reports: {clinvar_sig}", 0.8, True
            )
    ...
    def ba1(self, data: DataRequirements) -> EvidenceResult:
        ...
        return EvidenceResult(
            "BA1",
            False,
            "Frequency below BA1 threshold (AF={data.gnomad_af:.6f})",
            0.0,
            True,
        )
    ...
    def bs1(self, data: DataRequirements) -> EvidenceResult:
        ...
        if data.gnomad_af > self.thresholds.bs1_af:
            return EvidenceResult(
                "BS1",
                True,
                "Higher than expected frequency (AF={data.gnomad_af:.4f})",
                0.9,
                True,
            )
    ...
    def bs2(self, data: DataRequirements) -> EvidenceResult:
        ...
        return EvidenceResult(
            "BS2", False, "Healthy adult observation data not available", 0.0, False
        )

    def bs3(self, data: DataRequirements) -> EvidenceResult:
        """BS3: Functional studies show no deleterious effect."""
        if not data.functional_study_result:
            return EvidenceResult("BS3", False, "No functional study data", 0.0, False)
        ...
        if result == FunctionalStudyResult.BENIGN.value:
            return EvidenceResult(
                "BS3", True, "Functional studies show benign effect", 0.95, True
            )
    ...
    def bs4(self, data: DataRequirements) -> EvidenceResult:
        ...
        return EvidenceResult(
            "BS4", False, "Segregation analysis not implemented", 0.0, False
        )
    ...
    def bp1(self, consequence: str, gene: str) -> EvidenceResult:
        ...
        if is_missense and is_lof_gene:
            return EvidenceResult(
                "BP1", True, "Missense in LOF gene {gene}", 0.65, True
            )

        return EvidenceResult(
            "BP1", False, "Not missense in LOF gene (gene={gene})", 0.0, True
        )
    ...
    def bp2(self, data: DataRequirements) -> EvidenceResult:
        ...
        if data.is_in_trans_benign is None:
            return EvidenceResult("BP2", False, "Phase data not available", 0.0, False)

        if data.is_in_trans_benign:
            return EvidenceResult(
                "BP2", True, "In trans with pathogenic in dominant gene", 0.8, True
            )
    ...
    def bp3(self, consequence: str, data: DataRequirements) -> EvidenceResult:
        ...
        is_inframe = (
            "inframe" in consequence.lower() or "in_frame" in consequence.lower()
        )

        if data.in_repetitive_region is None:
            return EvidenceResult(
                "BP3", False, "Repetitive region data not available", 0.0, False
            )

        if is_inframe and data.in_repetitive_region:
            return EvidenceResult(
                "BP3", True, "In-frame indel in repetitive region", 0.7, True
            )
    ...
    def bp4(self, data: DataRequirements) -> EvidenceResult:
        ...
        if data.polyphen_score is not None:
            scores.append(
                ("PolyPhen", data.polyphen_score <= self.thresholds.polyphen_benign)
            )
        ...
        if not scores:
            return EvidenceResult(
                "BP4", False, "No computational predictions", 0.0, False
            )
        ...
        if benign_count >= 2:
            return EvidenceResult(
                "BP4", True, "Multiple tools predict benign", 0.6, True
            )
    ...
    def bp5(self, data: DataRequirements) -> EvidenceResult:
        ...
        if data.alternate_diagnosis is None:
            return EvidenceResult(
                "BP5", False, "No alternate diagnosis data", 0.0, False
            )

        if data.alternate_diagnosis:
            return EvidenceResult(
                "BP5", True, "Alternate molecular basis identified", 0.85, True
            )
    ...
    def bp6(self, clinvar_sig: str, data: DataRequirements) -> EvidenceResult:
        ...
        if not clinvar_sig or not clinvar_sig.strip():
            if data.clinvar_benign_reported is None:
                return EvidenceResult(
                    "BP6", False, "No ClinVar significance data", 0.0, False
                )
        ...
        if is_benign:
            return EvidenceResult(
                "BP6", True, f'ClinVar reports: {clinvar_sig or "benign"}', 0.75, True
            )
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print(
        "Use via: from varidex.core.classifier.acmg_evidence_full import ACMGEvidenceEngine"
    )
