...
class ACMGEvidenceEngine:
    ...
    def ps1(self, gene: str, aa_change: Optional[str], data: DataRequirements) -> EvidenceResult:
        ...
        if data.clinvar_pathogenic_same_aa is None:
            return EvidenceResult(
                "PS1", False, "No data on pathogenic variants at same position", 0.0, False
            )
    ...
    def ps2(self, data: DataRequirements) -> EvidenceResult:
        ...
        if data.de_novo_confirmed is None:
            return EvidenceResult("PS2", False, "No parental data available", 0.0, False)

        if data.de_novo_confirmed:
            return EvidenceResult("PS2", True, "Confirmed de novo with parental testing", 1.0, True)
    ...
    def ps4(self, data: DataRequirements) -> EvidenceResult:
        ...
        return EvidenceResult("PS4", False, "Case-control data not available", 0.0, False)
    ...
    def pm3(self, data: DataRequirements) -> EvidenceResult:
        ...
        if data.is_in_trans_pathogenic:
            return EvidenceResult("PM3", True, "In trans with pathogenic variant", 0.9, True)
    ...
    def pm4(self, consequence: str) -> EvidenceResult:
        ...
        length_change = {"inframe_deletion", "inframe_insertion", "stop_lost", "in_frame"}

        has_change = any(t in consequence.lower() for t in length_change)

        if has_change:
            return EvidenceResult("PM4", True, "Protein length changing variant", 0.75, True)
    ...
    def pm5(self, aa_change: Optional[str], data: DataRequirements) -> EvidenceResult:
        ...
        return EvidenceResult("PM5", False, "Pathogenic variant database not available", 0.0, False)
    ...
    def pp1(self, data: DataRequirements) -> EvidenceResult:
        ...
        return EvidenceResult("PP1", False, "Segregation analysis not implemented", 0.0, False)
    ...
    def pp3(self, data: DataRequirements) -> EvidenceResult:
        ...
        if data.sift_score is not None:
            scores.append(
                ("SIFT", data.sift_score < self.thresholds.sift_deleterious, data.sift_score)
            )
        ...
        if data.cadd_score is not None:
            scores.append(
                ("CADD", data.cadd_score > self.thresholds.cadd_pathogenic, data.cadd_score)
            )

        if data.revel_score is not None:
            scores.append(
                ("REVEL", data.revel_score > self.thresholds.revel_pathogenic, data.revel_score)
            )

        if not scores:
            return EvidenceResult("PP3", False, "No computational predictions", 0.0, False)
    ...
    def bs4(self, data: DataRequirements) -> EvidenceResult:
        ...
        return EvidenceResult("BS4", False, "Segregation analysis not implemented", 0.0, False)
    ...
    def bp3(self, consequence: str, data: DataRequirements) -> EvidenceResult:
        ...
        if data.in_repetitive_region is None:
            return EvidenceResult("BP3", False, "Repetitive region data not available", 0.0, False)

        if is_inframe and data.in_repetitive_region:
            return EvidenceResult("BP3", True, "In-frame indel in repetitive region", 0.7, True)
    ...
    def bp4(self, data: DataRequirements) -> EvidenceResult:
        ...
        if not scores:
            return EvidenceResult("BP4", False, "No computational predictions", 0.0, False)

        benign_count = sum(1 for _, pred in scores if pred)

        if benign_count >= 2:
            return EvidenceResult("BP4", True, "Multiple tools predict benign", 0.6, True)
    ...
    def bp5(self, data: DataRequirements) -> EvidenceResult:
        ...
        if data.alternate_diagnosis is None:
            return EvidenceResult("BP5", False, "No alternate diagnosis data", 0.0, False)

        if data.alternate_diagnosis:
            return EvidenceResult("BP5", True, "Alternate molecular basis identified", 0.85, True)
    ...
    def bp6(self, clinvar_sig: str, data: DataRequirements) -> EvidenceResult:
        ...
        if not clinvar_sig or not clinvar_sig.strip():
            if data.clinvar_benign_reported is None:
                return EvidenceResult("BP6", False, "No ClinVar significance data", 0.0, False)
    ...
    def bp7(self, consequence: str, data: DataRequirements) -> EvidenceResult:
        ...
        if data.spliceai_score is None:
            return EvidenceResult(
                "BP7", False, "Synonymous but no splice prediction available", 0.0, False
            )
