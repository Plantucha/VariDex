...
class PS1ClassifierOptimized:
    ...
    def _build_pathogenic_index(self) -> Set[Tuple[str, str]]:
        ...
        if cache_path.exists():
            ...
            print(f"PS1: ✓ Loaded {len(pathogenic_set):,} pathogenic AA changes (cached)")
        ...
        path_mask = (
            self.clinvar_df["clinical_sig"]
            .str.contains("Pathogenic|Likely_pathogenic", case=False, na=False)
        )
        
        review_mask = (
            self.clinvar_df["review_status"]
            .str.contains("criteria_provided|reviewed_by_expert|practice_guideline", 
                         case=False, na=False)
        )
        ...
        pathogenic_set = set()
        valid_mask = (
            pathogenic_df["gene"].notna() & 
            pathogenic_df["protein_change"].notna()
        )
    ...
    def apply_ps1(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        print(f"⭐ PS1: {ps1_count} exact AA matches with pathogenic variants ({ps1_pct:.1f}%)")
