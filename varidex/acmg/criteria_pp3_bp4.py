...
class PP3_BP4_Classifier:
    ...
    def apply_pp3_bp4(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        print(f"PP3/BP4: Using {len(pred_cols)} prediction tools: {', '.join(pred_cols)}")
        ...
        for idx, row in df.iterrows():
            ...
            # Apply consensus (need 3+ agreeing OR 2+ if only 2-3 tools available)
            min_votes = min(self.thresholds['consensus_required'], max(2, total_tools - 1))
            
            if deleterious_votes >= min_votes:
                df.at[idx, "PP3"] = True
                pp3_count += 1
            elif benign_votes >= min_votes:
                df.at[idx, "BP4"] = True
                bp4_count += 1
        ...
        print(f"⭐ PP3: {pp3_count} variants with deleterious predictions ({pp3_pct:.1f}%)")
        if pp3_count > 0:
    ...
    def _get_prediction_columns(self, df: pd.DataFrame) -> list:
        ...
        possible_cols = [
            ...
            'PolyPhen_score', 'PolyPhen_pred', 'Polyphen2_HDIV_score',
            'CADD_phred', 'CADD_raw',
            'REVEL_score',
