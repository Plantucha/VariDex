...
def detect_build(df: pd.DataFrame, sample_size: int = 1000) -> str:
    ...
    if not all(col in df.columns for col in required_cols):
        logger.warning(f"Missing columns for build detection, assuming GRCh37")
        return "GRCh37"
    ...
    if grch37_votes > grch38_votes:
        logger.info(f"🔍 Detected build: GRCh37 (votes: 37={grch37_votes}, 38={grch38_votes})")
        return "GRCh37"
    elif grch38_votes > grch37_votes:
        logger.info(f"🔍 Detected build: GRCh38 (votes: 37={grch37_votes}, 38={grch38_votes})")
        return "GRCh38"
    else:
...
def download_chain_file(from_build: str, to_build: str, cache_dir: Path = Path(".cache")) -> Optional[Path]:
    """
    ...
    url = f"http://hgdownload.soe.ucsc.edu/goldenPath/{from_ucsc}/liftOver/{chain_gz}"
...
def ensure_build_match(
    ...
    logger.info(f"\n{'='*70}")
    logger.info(f"BUILD COMPATIBILITY CHECK")
    logger.info(f"{'='*70}")
    logger.info(f"  User data: {user_build}")
    logger.info(f"  ClinVar:   {clinvar_build}")
    
    if user_build == clinvar_build:
        logger.info(f"  ✓ Builds match - no conversion needed")
        logger.info(f"{'='*70}\n")
        return user_df, user_build
    
    # Build mismatch detected
    logger.warning(f"  ⚠️  Build mismatch detected!")
    
    if not auto_convert:
        logger.warning(
            f"  Auto-conversion disabled. Results may be inaccurate.\n"
            f"  Consider using matching builds or enable auto_convert=True"
        )
        logger.info(f"{'='*70}\n")
        return user_df, user_build
    ...
    logger.info(f"\n{'='*70}")
    logger.info(f"LIFTOVER VALIDATION")
    logger.info(f"{'='*70}")
...
if __name__ == "__main__":
    ...
    print(f"✓ Same build test: positions unchanged")
