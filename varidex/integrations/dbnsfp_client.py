...
class DbNSFPClient:
    ...
    def __init__(
        ...
        logger.info(
            f"DbNSFPClient initialized: VEP={vep_url}, cache={enable_cache}, rate_limit={rate_limit}"
        )
    ...
    def _parse_vep_response(
        ...
        try:
            ...
            if len(available) < 3:
                logger.warning(
                    f"Limited algorithm coverage for {variant_id}: only {', '.join(available)} available"
                )
