...
CHROMOSOME_MAX_POSITIONS = {'1': 250_000_000, '2': 243_000_000, '3': 198_000_000, '4': 191_000_000, '5': 181_000_000, '6': 171_000_000, '7': 160_000_000, '8': 146_000_000, '9': 142_000_000, '10': 136_000_000, '11': 135_000_000, '12': 134_000_000, '13': 115_000_000, '14': 107_000_000, '15': 103_000_000, '16': 90_500_000, '17': 84_000_000, '18': 80_500_000, '19': 59_000_000, '20': 64_500_000, '21': 48_000_000, '22': 52_000_000, 'X': 156_000_000, 'Y': 57_000_000, 'MT': 17_000}
CLINVAR_COLUMNS = {'rsid': ['#AlleleID', 'RS# (dbSNP)', 'rsid'], 'gene': ['GeneSymbol', 'Gene(s)', 'gene'], 'clinical_sig': ['ClinicalSignificance', 'clinical_significance'], 'review_status': ['ReviewStatus', 'review_status'], 'variant_type': ['Type', 'VariantType', 'variant_type'], 'chromosome': ['Chromosome', 'chromosome', 'chr'], 'position': ['Start', 'PositionVCF', 'position', 'pos']}
...
def detect_clinvar_file_type(filepath: Path) -> str:
    ...
    try:
        ...
        vcf_markers = sum(['chrom' in header_lower, 'pos' in header_lower or 'position' in header_lower, 'ref' in header_lower, 'alt' in header_lower])
        return 'vcf_tsv' if vcf_markers >= 3 else 'variant_summary'
    except Exception as e:
        raise FileProcessingError(f"Failed to detect file type", context={'file': str(filepath), 'error': str(e)})


def validate_chromosome_consistency(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize chr naming (chr1→1, M→MT, NC_000001→1)."""
    if 'chromosome' not in df.columns: return df
    df = df.copy()
    ...
    df['chromosome'] = df['chromosome'].apply(map_nc).replace({'23': 'X', '24': 'Y', 'M': 'MT'}).str.upper()
    return df


def validate_position_ranges(df: pd.DataFrame) -> pd.DataFrame:
    """VECTORIZED: Validate positions within chr bounds."""
    if 'chromosome' not in df.columns or 'position' not in df.columns: return df
    orig_len = len(df)
    ...
    for chrom, max_pos in CHROMOSOME_MAX_POSITIONS.items():
        ...
        if chrom_mask.any(): logger.warning(f"{chrom_mask.sum()} variants on {chrom} exceed max {max_pos}")
        invalid_mask |= chrom_mask
    df = df[~invalid_mask].reset_index(drop=True)
    if orig_len > len(df): logger.info(f"Filtered {orig_len - len(df)} invalid positions")
    return df


def split_multiallelic_vcf(df: pd.DataFrame) -> pd.DataFrame:
    """Split ALT=A,G → 2 rows."""
    if df is None or len(df) == 0 or 'ALT' not in df.columns: return df
    try:
        multiallelic = df[df['ALT'].str.contains(',', na=False)]
        if len(multiallelic) == 0: return df
        ...
        for idx, row in multiallelic.iterrows():
            try:
                for alt in [a.strip() for a in str(row['ALT']).split(',') if a.strip()]:
                    new_row = row.copy()
                    new_row['ALT'], new_row['alt_allele'] = alt, alt.upper()
                    split_rows.append(new_row)
            except Exception as e:
                failed += 1
                logger.error(f"Split failed row {idx}: {e}")
        if failed > 0: logger.warning(f"{failed}/{len(multiallelic)} splits failed")
        if split_rows:
...
def load_clinvar_vcf(filepath: Path, checkpoint_dir: Optional[Path] = None) -> pd.DataFrame:
    ...
    print(f"\n[VCF] {filepath.name}")
    try:
        df = pd.read_csv(filepath, sep='\t', comment='#', names=['CHROM', 'POS', 'ID', 'REF', 'ALT', 'QUAL', 'FILTER', 'INFO'], dtype={'CHROM': str, 'POS': 'Int64', 'ID': str, 'REF': str, 'ALT': str}, low_memory=False, on_bad_lines='skip')
        if len(df) == 0: raise ValidationError("VCF empty", context={'file': str(filepath)})
        def parse_info(info):
            result = {'CLNSIG': '', 'CLNREVSTAT': ''}
            if pd.isna(info): return result
        ...
        df['position'], df['ref_allele'], df['alt_allele'] = df['POS'], df['REF'].str.upper(), df['ALT'].str.upper()
        ...
        orig_len = len(df)
        df = df[df['chromosome'].isin(VALID_CHROMOSOMES)].drop_duplicates('coord_key')
...
def load_clinvar_vcf_tsv(filepath: Path, checkpoint_dir: Optional[Path] = None) -> pd.DataFrame:
    ...
    try:
        ...
        if len(df) == 0: raise ValidationError("TSV empty", context={'file': str(filepath)})
        col_map = {}
        for target, candidates in {'chromosome': ['chromosome', 'chrom', 'chr'], 'position': ['position', 'pos'], 'ref_allele': ['ref', 'ref_allele', 'reference'], 'alt_allele': ['alt', 'alt_allele', 'alternate'], 'gene': ['gene', 'gene_symbol'], 'clinical_sig': ['clinical_significance', 'clin_sig', 'significance']}.items():
        ...
        df = normalize_dataframe_coordinates(df)
        orig_len = len(df)
        df = df.drop_duplicates('coord_key').reset_index(drop=True)
        if orig_len > len(df): print(f"  Deduped: {orig_len:,} → {len(df):,}")
...
def load_variant_summary(filepath: Path, checkpoint_dir: Optional[Path] = None) -> pd.DataFrame:
    ...
    try:
        ...
        if sep is None: raise ValidationError("Unknown separator", context={'file': str(filepath)})
        df = pd.read_csv(filepath, sep=sep, low_memory=False)
        if len(df) == 0: raise ValidationError("Summary empty", context={'file': str(filepath)})
        col_map = {}
        ...
        if 'rsid' in df.columns:
            ...
            if orig_len > len(df): print(f"  Filtered rsIDs: {orig_len:,} → {len(df):,}")
        ...
        if 'rsid' in df.columns and df['rsid'].duplicated().any():
            agg_dict = {'clinical_sig': lambda x: ' | '.join(x.dropna().astype(str).unique()), 'gene': lambda x: ';'.join(x.dropna().astype(str).unique())}
            for col in ['review_status', 'variant_type']:
                if col in df.columns: agg_dict[col] = lambda x: ' | '.join(x.dropna().astype(str).unique())
...
def load_clinvar_file(filepath: Path, **kwargs) -> pd.DataFrame:
    """
    Auto-detect and load ClinVar file (main entry).
    Supports: VCF, VCF-TSV, variant_summary.txt
    Returns DataFrame with coord_key.
    """
    try:
        file_type = detect_clinvar_file_type(filepath)
        print(f"\n{'='*70}\nLOADING {file_type.upper()}: {filepath.name}\n{'='*70}")
        loaders = {'vcf': load_clinvar_vcf, 'vcf_tsv': load_clinvar_vcf_tsv, 'variant_summary': load_variant_summary}
        loader = loaders.get(file_type)
        if loader is None: raise ValueError(f"Unknown file type: {file_type}")
