...
def sample_variants():
    ...
    return [
        AnnotatedVariant(
            chr="1", pos=100, ref="A", alt="T", acmg_class="P", gnomad_af=0.01
        ),
        AnnotatedVariant(
            chr="2", pos=200, ref="C", alt="G", acmg_class="B", gnomad_af=0.05
        ),
        AnnotatedVariant(
            chr="1", pos=300, ref="G", alt="A", acmg_class="LB", gnomad_af=0.1
        ),
    ]
...
class TestReportGenerator:
    ...
    @pytest.mark.parametrize(
        "output_path", ["report.html", "variants.tsv", "data.json", "export.csv"]
    )
