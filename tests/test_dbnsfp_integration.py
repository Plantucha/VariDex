...
class TestPredictionScore:
    ...
    def test_count_deleterious_cadd(self) -> None:
        score = PredictionScore(variant_id="1-12345-A-G", cadd_phred=25.0)
        assert score.count_deleterious() == 1

    def test_count_deleterious_multiple(self) -> None:
        score = PredictionScore(
            variant_id="1-12345-A-G",
            sift_score=0.01,
            polyphen_score=0.9,
    ...
    def test_count_benign_sift(self) -> None:
        score = PredictionScore(
...
class TestValidationHelpers:
    ...
    def test_validate_not_none_fail(self) -> None:
...
class TestDbNSFPClient:
    ...
    def test_cache_operations(self) -> None:
        ...
        assert cached.sift_score == 0.01  # type: ignore[union-attr]
        client.clear_cache()
        assert client._get_from_cache("1-12345-A-G") is None

    @patch("requests.Session.get")
    def test_get_predictions_not_found(self, mock_get: Mock) -> None:
        mock_resp = Mock()
    ...
    @patch("requests.Session.get")
    def test_get_predictions_found(self, mock_get: Mock) -> None:
        mock_resp = Mock()
        mock_resp.json.return_value = [
            {
        ...
        assert result.sift_score == 0.01  # type: ignore[union-attr]
        assert result.polyphen_score == 0.95  # type: ignore[union-attr]
        assert result.cadd_phred == 28.5  # type: ignore[union-attr]
...
class TestComputationalPredictionService:
    ...
    def test_init_with_predictions(self) -> None:
    ...
    def test_check_bp4_sufficient(self) -> None:
        service = ComputationalPredictionService(enable_predictions=False)
        evidence = ComputationalEvidence(
            deleterious_count=0,
            benign_count=3,
    ...
    def test_check_bp4_deferred_to_pp3(self) -> None:
        service = ComputationalPredictionService(enable_predictions=False)
        evidence = ComputationalEvidence(
...
class TestACMGClassifierV8:
    ...
    def test_init_without_predictions(self) -> None:
    ...
    def test_get_enabled_codes_with_predictions(self) -> None:
