...
def load_user_variants(filepath: Path) -> pd.DataFrame:
    ...
    if suffix == '.csv':
        ...
        df = pd.read_csv(filepath, sep='\t', comment='#', header=None,
                        names=['chromosome', 'position', 'id', 'ref_allele',
                               'alt_allele', 'qual', 'filter', 'info'])
    else:
    ...
    logger.info(f"   Chromosomes: {sorted(df['chromosome'].unique(), key=lambda x: (not x.isdigit(), x))}")
    
    return df


def match_with_clinvar(
...
def annotate_with_gnomad(
    ...
    with GnomADAnnotator(config) as annotator:
        ...
        logger.info(f"   Found in gnomAD: {stats['found_in_gnomad']:,}/{stats['total_variants']:,}")
...
def analyze_pathogenic_variants(
    ...
    if 'gnomad_af' in pathogenic.columns:
        ...
        # Rare (AF < 0.1%)
        rare = pathogenic[
            pathogenic['gnomad_af'].notna() & 
            (pathogenic['gnomad_af'] < 0.001)
        ]
...
def main():
    ...
    try:
        ...
        annotated = annotate_with_gnomad(
            matched,
            args.gnomad_dir,
            max_af=args.max_af
        )
